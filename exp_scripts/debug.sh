...
i=0
CUDA_VISIBLE_DEVICES=4,5,6,7 python main.py --root_path $root_path --annotation_path $annotation_path --video_path $video_path --sample_duration $sample_duration --result_path results  --dataset $dataset --n_classes $n_classes --ft_begin_module fc --model resnet --model_depth 50 --batch_size $batch_size --n_threads 4 --checkpoint 200 > logs/sknet_tmp$i.txt
